...
contextBridge.exposeInMainWorld('authlock', {
  // 嵌入指纹
  embedFingerprint: async (inputPath, outputPath, payload) => {
    return await ipcRenderer.invoke('embed-fingerprint', inputPath, outputPath, payload);
  },
  ...
  // 保存指纹信息
  saveFingerprintInfo: async (fingerprintData, outputPath) => {
    return await ipcRenderer.invoke('save-fingerprint-info', fingerprintData, outputPath);
  }
});